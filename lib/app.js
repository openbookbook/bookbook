...
app.post('/api/ballots', async (req, res) => {
  try {
    ...
    INSERT INTO ballots (admin_code, vote_code, name)
    VALUES ($1, $2, $3)
    RETURNING id, name, admin_code as "adminCode", vote_code as "voteCode";
    `, [ballot.adminCode, ballot.voteCode, ballot.name]);
...
app.put('/api/ballots/:id', async (req, res) => {
  try {
    ...
    const data = await client.query(`
    UPDATE ballots
    SET name = $1, admin_code = $2, vote_code = $3
    WHERE id = $4
    RETURNING id, name, admin_code as "adminCode", vote_code as "voteCode";
    `, [ballot.name, ballot.adminCode, ballot.vodeCode, req.params.id]);
...
app.get('/api/ballots/:id', async (req, res) => {
  try {
    const data = await client.query(`
    SELECT id, admin_code as "adminCode", name, vote_code as "voteCode"
    FROM ballots
    WHERE ballot_id = $1
    `, [req.params.id]);
...
app.post('/api/:ballotid/votes', async (req, res) => {
  try {
    const vote = req.body;
    const data = await client.query(`
    ...
    `, [vote.userId, req.params.ballotid, vote.vote]);
...
app.post('/api/:ballotid/users', async (req, res) => {
  try {
    const user = req.body;
    const data = await client.query(`
    ...
    `, [user.username, req.params.ballotid, user.password]);
